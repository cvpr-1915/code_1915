...
class StyleBlenderWithADEDataset(torch.utils.data.Dataset):
...
#            opt.load_size = 286
        ...
        ade20k_label_dir = os.path.join(opt.ade20k_dir, 'labels_blender')
        self.ade20k_images = sorted(glob.glob(
            os.path.join(ade20k_image_dir, '*.jpg')))
        self.ade20k_labels = sorted(glob.glob(
            os.path.join(ade20k_label_dir, '*.png')))
    ...
    def get_coordinate_image(self, path):
        ...
        if self.opt.quantize != 0:
            coordinate_image_tensor = torch.round(coordinate_image_tensor * self.opt.quantize) / self.opt.quantize
    ...
    def __getitem__(self, idx):
        ...
        ade20k_label_path = self.ade20k_labels[ade20k_random_idcs]
        ade20k_image = Image.open(ade20k_image_path).convert('RGB')
        ade20k_label = Image.open(ade20k_label_path)
        ade20k_image, ade20k_label = self.transforms(ade20k_image, ade20k_label)
        ade20k_label = ade20k_label * 255
        ...
        return {"label": fake_label, "coord_image": coord_image,
                ...
                "ade20k_image": ade20k_image,
                "ade20k_label": ade20k_label, }
