...
class SPADE_with_coord(nn.Module):
    def __init__(self, opt, norm_nc, label_nc):
        super().__init__()
        self.first_norm = get_norm_layer(opt, norm_nc)
        ks = opt.spade_ks
    ...
    def forward(self, x, segmap, coord):
        normalized = self.first_norm(x)
        segmap = F.interpolate(segmap, size=x.size()[2:], mode='nearest')
        coord = F.interpolate(coord, size=x.size()[2:], mode='bilinear', align_corners=True)
        seg_coord = torch.cat([segmap, coord], dim=1)
        actv = self.mlp_shared(seg_coord)
        gamma = self.mlp_gamma(actv)
        beta = self.mlp_beta(actv)
        out = normalized * (1 + gamma) + beta
        return out
