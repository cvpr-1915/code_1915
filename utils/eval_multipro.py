...
colors = loadmat('utils/color150.mat')['colors']
...
def main(cfg, gpus):
    ...
    # modified
    blender_class_list = [31, 8, 68, 45, 4, 37, 41, 58, 24, 16, 1, 9, 15, 23]
    blender_iou = []
    ...
    for i, _iou in enumerate(iou):
        ...
        if (i + 1) in blender_class_list:
            print('class [{}], IoU: {:.4f}'.format(i, _iou))
            blender_iou.append(_iou)

    #    print('[Eval Summary]:')
    #    print('Mean IoU: {:.4f}, Accuracy: {:.2f}%'
    #          .format(iou.mean(), acc_meter.average()*100))

    miou = np.mean(blender_iou)
    print('Evaluation Done!')
    print('Blender class mIoU: {:.4f}'.format(miou))
    print(miou)
    return miou
